...
def _looks_like_datetime(series: pd.Series) -> bool:
    ...
    if pd.api.types.is_numeric_dtype(s):
        str_len = s.astype(str).str.len()
        return str_len.between(10, 13).mean() > 0.5

    if pd.api.types.is_string_dtype(s):
        candidates = s.str.contains(r"(?:\d{2,4}[-/.]\d{1,2}[-/.]\d{1,4}|[Jj]an|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)", regex=True)
        return candidates.mean() > 0.5
